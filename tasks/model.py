...
class SprintTask(BaseModel):
    ...
    sp: float = Field(..., ge=0, description="Story Points")

    business_value: float = Field(..., ge=0, description="Бизнес-ценность")

    has_escalation: bool = Field(default=False, description="Есть ли эскалация")

    escalation_count: int = Field(default=0, ge=0, description="Количество эскалаций")
    ...
    rice: Optional[float] = Field(None, ge=0, description="RICE score")

    quota: Optional[float] = Field(None, ge=0, description="Квота/лимит")
